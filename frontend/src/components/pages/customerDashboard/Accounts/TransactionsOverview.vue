<template>
  <div class="dashboard">
    ...
    <div class="main-content">
      <header class="header">
        <h1>{{ state.accountType }} Account Transactions</h1>
        <div class="user-info">
      ...
      <TransactionList v-if="state.transactions" :transactions="state.transactions" :account-id="state.accountId" />
    </div>
  </div>
</template>

<script setup>
import TransactionList from "../../../containers/customerDashboard/accounts/TransactionList.vue";
...
import {getAllTransactionsByAccount} from "@/services/TransactionService.js";
...
onMounted(async () => {
  ...
  if (accountId) {
    ...
    state.transactions = await getAllTransactionsByAccount(accountId);
  }
});
</script>
